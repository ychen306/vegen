...
VectorPack *VectorPackContext::createVectorPack(
    ...
    BitVector Depended, const InstBinding *Producer, TargetTransformInfo *TTI) const {
  VectorPackCache::GeneralPackKey Key { 
    decltype(VectorPack::Matches)(Matches.begin(), Matches.end()),
    Producer };
...
VectorPack *VectorPackContext::createLoadPack(ArrayRef<LoadInst *> Loads,
                                              BitVector Elements,
                                              BitVector Depended, TargetTransformInfo *TTI) const {
...
VectorPack *VectorPackContext::createPhiPack(ArrayRef<PHINode *> PHIs, TargetTransformInfo *TTI) const {
  ...
  if (VP) return VP.get();
  VP.reset(new VectorPack(this, PHIs, Elements, BitVector(getNumValues()), TTI));
  return VP.get();
}
